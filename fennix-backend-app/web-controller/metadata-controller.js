...
const {METADATA_CONTROLLER} = require('../util-module/util-constants/fennix-controller-constants');
...
router.get(METADATA_CONTROLLER.METADATA_WIDGET_FILTER, (req, res) => {
    let returnObj;
    returnObj = metadataBusiness.getFilterMetadataBusiness(req, 'roleCardWidgetId');
    returnObj.then((response) => {
        res.send(response);
    })
});
