...
const commonCreateRequestUserDetailsBusiness = (request, userId) => {
    const date = new Date();
    request.createdBy = userId;
    request.createdDate = `${date.getDate()}${(date.getMonth() + 1)}${date.getFullYear()}_${date.getHours()}_${date.getMinutes()}_${date.getSeconds()}`;
    return request;
};

const commonUpdateRequestUserDetailsBusiness = (request, userId) => {
    const date = new Date();
    request.updatedBy = userId;
    request.updateDate = `${date.getDate()}${(date.getMonth() + 1)}${date.getFullYear()}_${date.getHours()}_${date.getMinutes()}_${date.getSeconds()}`;
    return request;
};

const commonResponseFormatterBusiness = (response, location) => {
    let responseObj = null;
    if (location) {
        switch (location.toLowerCase()) {
            case 'grid':
                responseObj = {};
                responseObj['gridData'] = [...response];
                break;
            case 'dropdown':
                responseObj = {};
                responseObj['dropdownData'] = [...response];
                break;
            default:
                responseObj = Object.prototype.toString.call(response) === '[object Array]' ? [...response] : {...response};
                break;
        }
    }
    return responseObj;
...
module.exports = {
    dropDownBusiness,
    commonResponseFormatterBusiness,
    commonUpdateRequestUserDetailsBusiness,
    commonCreateRequestUserDetailsBusiness,
